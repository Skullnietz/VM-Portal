...
class AdminController extends Controller
{
    ...
    public function importCSV(Request $request)
    {
        ...
        if ($request->hasFile('csv_file')) {
            ...
            if (empty($path)) {
                Log::error('ImportCSV: El path del archivo está vacío.', ['original_name' => $file->getClientOriginalName()]);
                return response()->json(['status' => 'error', 'message' => 'No se pudo obtener la ruta del archivo temporal. Verifique la configuración de PHP (upload_tmp_dir) o permisos.'], 500);
            }
