...
@section('content')
<style>
  :root{ --urv-verde:#0E7E2D; --urv-azul:#0F1934 }
  .card{background:#fff;border:1px solid #e6eaf2;border-radius:14px;box-shadow:0 6px 16px rgba(15,25,52,.06)}
  .mini-progress{height:10px;background:#f3f6fb;border-radius:999px;overflow:hidden}
  .mini-progress>span{display:block;height:100%}
  .bar-consumido{background:var(--urv-azul)}
  .bar-disponible{background:var(--urv-verde)}
  table.dataTable tbody tr.alerta>td{ background:#fff4f4 !important; }
  .badge-chip{font-size:11px;padding:2px 8px;border-radius:999px;border:1px solid #d9e1ee;background:#f7f9fc}
  /* Barra unificada */
  .uni-bar{height:12px;background:#eef3f9;border-radius:999px;position:relative;overflow:hidden}
  .uni-bar .seg{position:absolute;left:0;top:0;bottom:0}
  .uni-bar .seg.consumido{background:#0F1934}
  .uni-bar .seg.disponible{background:#0E7E2D}
  .uni-bar .seg.excedido{background:#d63031} /* rojo para exceso */
  .uni-meta{font-size:12px;color:#6b7280}

  /* Chip de estado */
  .chip{display:inline-flex;align-items:center;gap:6px;padding:3px 8px;border-radius:999px;font-size:12px;border:1px solid #e3e8f3}
  .chip.ok{background:#f2fbf5;color:#136f2b;border-color:#d7f0df}
  .chip.warn{background:#fff9ed;color:#a05b00;border-color:#ffe4b5}
  .chip.bad{background:#fff1f1;color:#9b1c1c;border-color:#ffd6d6}

  /* KPIs */
  .kpi{background:#fff;border:1px solid #e6eaf2;border-radius:12px;padding:12px}
  .kpi .t{font-size:12px;color:#6b7280}
  .kpi .v{font-size:18px;font-weight:600}

    .chart-h-110{ height:200px; } /* Ajusta aquí la altura deseada */
  .chart-h-150{ height:150px; } /* (opcional) otra altura */
  .chart-h-110 canvas,
  .chart-h-150 canvas{ width:100% !important; height:100% !important; display:block; }
</style>
  ...
  <div class="card p-3 mb-3">
    ...
    <form id="filtro" class="row g-3">
      ...
      <div class="col-sm-4 d-flex align-items-end gap-2">
        ...
        <button class="btn btn-outline-secondary" type="button" id="btnTodos">Todos</button>
      </div>

      <div class="col-12 d-flex gap-3">
        <div class="badge-chip">
          <span style="display:inline-block;width:10px;height:10px;background:var(--urv-azul);border-radius:2px;margin-right:6px"></span>
          Consumido
        </div>
        <div class="badge-chip">
          <span style="display:inline-block;width:10px;height:10px;background:var(--urv-verde);border-radius:2px;margin-right:6px"></span>
          Disponible
        </div>
      </div>
    </form>
...
$(function () {
  ...
  $('#btnTodos').on('click', function(){
    ...
    $('#NoEmpleado').val(''); table.ajax.reload();
  });
});
</script>
