...
@section('title', 'Permisos Globales — ' . $planta->Txt_Nombre_Planta)

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h1 class="m-0"><i class="fas fa-lock mr-2"></i>Permisos Globales</h1>
            <small class="text-muted">{{ $planta->Txt_Nombre_Planta }}</small>
        </div>
...
{{-- Panel de edición masiva (sticky, oculto hasta seleccionar) --}}
<div id="bulk-panel" class="card card-primary card-outline mb-2" style="display:none; position:sticky; top:60px; z-index:999;">
    <div class="card-body py-2">
        <div class="d-flex align-items-center flex-wrap gap-2">
            <span class="badge badge-primary mr-3" id="bulk-count" style="font-size:1em;">0 seleccionados</span>
            ...
            <select id="bulk-status" class="form-control form-control-sm mr-3" style="width:110px;">
                <option value="">— Estatus —</option>
                <option value="Alta">Alta</option>
                <option value="Baja">Baja</option>
            </select>

            <button id="bulk-apply" class="btn btn-sm btn-success mr-2">
...
$(document).ready(function () {
    ...
    var tsArticulo = new TomSelect('#filter-articulo', {
        ...
        load: function (query, callback) {
            $.getJSON('{{ route("permisos.globales.articulos", $planta->Id_Planta) }}', { q: query })
    ...
    var table = $('#permisos-globales-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("permisos.globales.data", $planta->Id_Planta) }}',
            type: 'POST',
            data: function (d) {
                d._token        = '{{ csrf_token() }}';
        ...
        drawCallback: function () {
            // Restaurar estado de checkboxes tras redibujado
    ...
    $('#bulk-apply').on('click', function () {
        ...
        Swal.fire({
            ...
            $.ajax({
                url: '{{ route("permisos.bulk.update") }}',
                method: 'POST',
                data: {
                ...
                success: function (response) {
                    if (response.success) {
                        Swal.fire('Listo', response.message, 'success').then(function () {
                            selectedIds.clear();
                            updateBulkPanel();
                            $('#bulk-frecuencia').val('');
                            $('#bulk-cantidad').val('');
    ...
    $('#permisos-globales-table').on('change', '.update-frecuencia', function () {
        ...
        $.post('/update-permiso-articulo/' + id, { field: 'Frecuencia', value: value, _token: '{{ csrf_token() }}' })
            ...
            .fail(function () { toastr.error('Error al actualizar frecuencia'); });
    });
    ...
    $('#permisos-globales-table').on('change', '.update-cantidad', function () {
        ...
        $.post('/update-permiso-articulo/' + id, { field: 'Cantidad', value: value, _token: '{{ csrf_token() }}' })
            ...
            .fail(function () { toastr.error('Error al actualizar cantidad'); });
    });
    ...
    $('#permisos-globales-table').on('click', '.toggle-status-global', function () {
        var $btn      = $(this);
        var id        = $btn.data('id');
        var newStatus = $btn.data('status') === 'Alta' ? 'Baja' : 'Alta';
